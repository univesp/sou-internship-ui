...
class StepDocuments extends Component {
  getValidationSchema = () => Yup.object().shape({
    files: Yup.object().shape({
      work: Yup.mixed()
              .test('fileSize', "Tamanho do arquivo não suportado, máximo 4mb", value => value && value.size <= FILE_SIZE)
              .test('fileFormat', "Formato não suportado", value => value && SUPPORTED_FORMATS.includes(value.type))
  ...
  render() {
    ...
    return (
      <Formik onSubmit={handleSubmit} validationSchema={this.getValidationSchema} initialValues={files}>
                ...
                >
                  {({ getRootProps, getInputProps, error }) => (
                    <DragDrop {...getRootProps()}>
                      ...
                      />
                      {console.log(errors.files && errors.files.work ? errors.files.work : null)}
                      <Field {...getInputProps()} />
