				...
				<Body>
					{process.map((item) => (
						<Row key={item.id}>
							...
							<Td align="left">
								<Status finalized={item.status}>Marcos Seilá das Quantas</Status>
							</Td>
							<Td>
								<Link to={`${url}/student/resume/${item.id}`}>Visualizar resumo</Link>
							</Td>
						</Row>
					))}
				</Body>
	...
	render() {
		...
		return (
			<Container>
				...
				<Actions>
					<Action to={`${url}/student/regulation`}>Aproveitamento de horas de estágio</Action>
